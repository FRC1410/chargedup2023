				...
				switch (targetNode) {
					case LEFT_YANKEE_NODE -> {
						if (camera.getTarget().getFiducialId() == 1 && waypointFlag)
							scheduler.scheduleAutoCommand(new OTFToPoint(drivetrain, pose.toPose2d(), RED_OUTSIDE_WAYPOINT, RED_LEFT_YANKEE_ANGLED_NODE));
						else
					...
					case RIGHT_YANKEE_NODE -> {
						if (camera.getTarget().getFiducialId() == 3 && waypointFlag)
							scheduler.scheduleAutoCommand(new OTFToPoint(drivetrain, pose.toPose2d(), RED_BARRIER_WAYPOINT, RED_RIGHT_YANKEE_ANGLED_NODE));
						else
				...
				switch (targetNode) {
					case LEFT_YANKEE_NODE -> {
						if (camera.getTarget().getFiducialId() == 6)
							scheduler.scheduleAutoCommand(new OTFToPoint(drivetrain, pose.toPose2d(), BLUE_BARRIER_WAYPOINT, BLUE_LEFT_YANKEE_ANGLED_NODE));
						else
					...
					case RIGHT_YANKEE_NODE -> {
						if (camera.getTarget().getFiducialId() == 8)
							scheduler.scheduleAutoCommand(new OTFToPoint(drivetrain, pose.toPose2d(), BLUE_OUTSIDE_WAYPOINT, BLUE_RIGHT_YANKEE_ANGLED_NODE));
						else
